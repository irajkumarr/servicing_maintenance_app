...
const acceptBooking = async (req, res) => {
  ...
  try {
    ...
    if (booking.status !== "pending") {
      return res.status(400).json({ status: false, message: "Booking already processed" });
    }
...
const handleUpdateBookingStatus = async (req, res) => {
  ...
  try {
    ...
    if (String(booking.provider) !== req.user.id) {
      return res.status(403).json({ status: false, message: "Not authorized to update this booking" });
    }
